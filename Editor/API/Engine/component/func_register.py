...
FUNCTION_REGISTRY = {
    ...
    "sin":        (CalcHandlers.execute, True), # 正弦波
    "cos":        (CalcHandlers.execute, True), # 余弦波 (あると便利)
    "tan":        (CalcHandlers.execute, True), # 正接 (あると便利)
    "radians":    (CalcHandlers.execute, True), # 度数法 -> ラジアン変換
    "degrees":    (CalcHandlers.execute, True), # ラジアン -> 度数法変換
