...
// Open/Close display drawer
document.getElementById("open-drawer").onclick = () =>
  drawer.classList.add("open");
document.getElementById("drawer-close").onclick = () =>
  drawer.classList.remove("open");

// Open/Close condition list drawer
document.getElementById("open-add-drawer").onclick = () => {
  addDrawer.classList.add("open");
};
document.getElementById("add-drawer-close").onclick = () =>
...
/* ================= Existing Conditions (List and Delete) ================= */
const existingConditionList = document.getElementById("existing-condition-list");
const existingHeader = document.querySelector("#existing-conditions .accordion-header");

existingHeader.onclick = () => existingConditionList.classList.toggle("open");

/**
 * Renders the list of settings (No changes to logic)
 */
function renderExistingConditions(settings) {
    existingConditionList.innerHTML = "";

    if (!Array.isArray(settings) || settings.length === 0) {
        existingConditionList.innerHTML = "<div style='font-size:12px;color:#888'>No conditions found</div>";
        return;
    }

    settings.forEach((c, idx) => {
        ...
        div.innerHTML = `
            <div style="display:flex;justify-content:space-between">
                <span>#${idx + 1}</span>
                <button class="condition-delete"
                    style="background:none;border:none;color:#f55;cursor:pointer"
                    data-index="${idx}">×</button>
            </div>
            <div>${c.name}</div>
            <div style="font-size:11px;color:#aaa;overflow:hidden;text-overflow:ellipsis;">${c.expression}</div>
        `;

        div.querySelector(".condition-delete").onclick = e => {
            e.stopPropagation();
            deleteCondition(idx);
        };

        existingConditionList.appendChild(div);
    });
}


async function deleteCondition(index) {
    if (!confirm(`Delete condition #${index + 1}?`)) return;

    try {
        const res = await fetch(`http://localhost:5000/api/settings/${index}`, {
            method: 'DELETE'
        });

        if (res.ok) {
            // Manually refresh the list since there is no broadcast system
            await reloadConditions(); 
            console.log(`Deleted setting ${index}`);
        } else {
...
async function reloadConditions() {
    try {
        ...
        if (res.ok) {
            ...
            renderExistingConditions(settings);
            showAlert("Reload complete", "info");
        } else {
            throw new Error("Fetch failed");
        }
    } catch (e) {
        console.error(e);
        showAlert("Failed to reload conditions", "alert");
    }
}

/* ================= Reload Existing Conditions Button ================= */
const reloadConditionsBtn = document.getElementById("reload-conditions-btn");

if (reloadConditionsBtn) {
    reloadConditionsBtn.onclick = (e) => {
...
if(socket){
  socket.onmessage = (event) => {
    ...
    if (data.type === 'settings') {
      renderExistingConditions(data.data);
      return;
    }
